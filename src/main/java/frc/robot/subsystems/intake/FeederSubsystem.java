...
/**
 * Feedersubsystem is responsible for feeding balls into the shoote We'll use
 * sensors to keep track of ball positions to hold some in the tower We can't
 * have too many in the hopper or they'll jam
 */
...
public class FeederSubsystem extends SubsystemBase {
  ...
  public FeederSubsystem() {
    ...
    for (WPI_VictorSPX feederMotors : new WPI_VictorSPX[] { m_hopperMotor, m_towerMotor }) {
      ...
      feederMotors.setNeutralMode(NeutralMode.Brake);
    }
  ...
  /**
   * Moves the ball up the tower. There are two options for doing this so we can
   * test which one works best. Comment out the one that you are not going to use.
   */
  ...
  public void index() {
    ...
    // TBD: if here are a buch of balls in the hopper we may have to stop the
    // hopper?
  ...
  public void checkIndexState() {
    ...
    if (topSensorTripped() && bottomSensorTripped()) {
