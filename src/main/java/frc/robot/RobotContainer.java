...
public class RobotContainer {
  ...
  public RobotContainer() {
    m_drivetrain = DrivetrainSubsystem.createFake();
    m_transmission = TransmissionSubsystem.createFake();
    m_flywheel = FlywheelSubsystem.createFake();
    m_hood = HoodSubsystem.createFake();
    m_turret = TurretSubsystem.create();
    m_feeder = FeederSubsystem.createFake();
    m_hopper = HopperSubsystem.createFake();
  ...
  private void configureShooterBindings() {
    m_driverOI.getEnableAutoTargetButton().whenPressed(new AutoTargetCommand(m_hood,
        m_turret, m_flywheel, LimelightUtility::getData, m_drivetrain::getHeading, m_driverOI.getFlywheelButton()));

    m_driverOI.getEnableAutoTargetButton().whenReleased(new InstantCommand(() -> {
